...
describe('DB_ROM class creating an object', async() => {
  ...
  it ('Should create a new instance of DB_ROM with data', async() => {
    expect(db_test.db.data).toEqual({ goods: [ { id: 0, name: 'Makaham Steel Sword', description: 'A sword made of the finest steel from Mahakam', material: Materials.MAKAHAM_STEEL, weight: 3.5, value: 500 } ], merchants: [ { id: 0, name: 'Yennefer', type: MerchantType.ALCHEMIST, location: Locations.KAER_MORHEN } ], customers: [], transactions: [] });
  });
